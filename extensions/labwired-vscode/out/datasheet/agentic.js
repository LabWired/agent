...
exports.DatasheetService = void 0;
/**
...
function findPdftotext() {
    for (const c of [
        ...
        try {
            const r = (0, child_process_1.spawnSync)(c, ["-v"], { encoding: "utf8" });
...
function extractPdf(pdfPath, outTxt) {
    const bin = findPdftotext();
    if (!bin) {
        return {
            ok: false,
            error: "pdftotext not found. Install poppler: brew install poppler (macOS) / apt install poppler-utils",
        };
    }
    // layout preserves columns better for register tables
    const r = (0, child_process_1.spawnSync)(bin, ["-layout", pdfPath, outTxt], { encoding: "utf8" });
...
function parseSections(text) {
    ...
    for (const line of lines) {
        const t = line.trim();
        if (/^(\d+(\.\d+){0,3}\s+|[A-Z][A-Z0-9 \-/]{8,}|Section\s+\d+|CHAPTER\s+\d+)/.test(t) &&
            t.length < 100) {
...
class DatasheetService {
    ensureDirs() {
        ...
        if (!fs.existsSync(readme)) {
            fs.writeFileSync(readme, `# Project datasheets (agentic — not vector RAG)
...
`, "utf8");
    ...
    readWindow(id, offset, length) {
        const meta = this.loadMeta(id);
        if (!meta)
            throw new Error(`Unknown datasheet: ${id}. Run extract first.`);
    ...
    readSection(id, sectionQuery) {
        const meta = this.loadMeta(id);
        if (!meta)
            throw new Error(`Unknown datasheet: ${id}`);
        const q = sectionQuery.toLowerCase();
        const sec = meta.sections.find((s) => s.title.toLowerCase().includes(q)) ||
            meta.sections.find((s) => s.title.toLowerCase() === q);
        if (!sec) {
            return (`Section not found: ${sectionQuery}\nAvailable (first 40):\n` +
                meta.sections
                    .slice(0, 40)
                    .map((s, i) => `  [${i}] ${s.title}`)
                    .join("\n"));
    ...
    grep(pattern, opts) {
        ...
        const linesOut = [];
        if (errors.length)
            linesOut.push("Extract warnings:", ...errors.map((e) => `  ${e}`), "");
        let hits = 0;
        for (const doc of docs) {
            if (opts?.id && doc.id !== opts.id)
                continue;
        ...
        if (!hits)
            linesOut.push(`No matches for /${pattern}/`);
        return linesOut.join("\n");
    }
...
exports.DatasheetService = DatasheetService;
