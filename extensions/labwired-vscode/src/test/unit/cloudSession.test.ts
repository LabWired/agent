...
suite("hosted conversation disclosure", () => {
  ...
  test("shows once per version in user state and redisplays a new version", () => {
    const env = { LABWIRED_HOME: root } as NodeJS.ProcessEnv;
    ...
    assert.ok(fs.existsSync(path.join(root, "state", "agent", "hosted-disclosure-v2")));
  });
  ...
  test("shows honestly when acknowledgement cannot be persisted", () => {
    ...
    assert.strictEqual(
      hostedDisclosureMessage({ LABWIRED_HOME: blocked }, "1"),
      HOSTED_DISCLOSURE
    );
  });
});
