...
export type ToolDef = {
  ...
  title: string;
  description: string;
  /** CLI argv after `labwired` (params as ${name}) */
  argv: string[];
  params?: ToolParam[];
  /** Category for UI grouping */
  group: "install" | "verify" | "hardware" | "project" | "agent";
  timeoutMs?: number;
};
...
export function listToolsByGroup(): Record<string, ToolDef[]> {
...
export function routeMessage(
  ...
  if (raw.startsWith("/")) {
    ...
    switch (cmd.toLowerCase()) {
      ...
      case "probe":
        ...
        if (rest[0] === "doctor") return { tool: "probe_doctor", params: {} };
        if (rest[0] === "chips")
          return {
            tool: "probe_chips",
        ...
        if (rest[0] === "flash" && rest[1]) {
          ...
          for (let i = 2; i < rest.length; i++) {
            if (rest[i] === "--chip" && rest[i + 1]) chip = rest[++i];
            else if (rest[i] === "--target" && rest[i + 1]) target = rest[++i];
          }
          if (!chip) return null;
          return {
            tool: "probe_flash",
      ...
      case "package":
        if (rest[0] === "path") return { tool: "package_path", params: {} };
      ...
      case "ds": {
        ...
        if (sub === "list") return { tool: "datasheet_list", params: {} };
        if (sub === "extract") return { tool: "datasheet_extract", params: {} };
        if (sub === "grep" && rest[1])
          return {
            tool: "datasheet_grep",
      ...
      case "gdb": {
        ...
        if (sub === "stop") return { tool: "debug_gdb_stop", params: {} };
        if (sub === "start" && rest[1])
          return {
  ...
  if (/^(run\s+)?doctor(\s+strict)?$/.test(lower))
    return lower.includes("strict")
      ? { tool: "doctor_strict", params: {} }
      : { tool: "doctor", params: {} };
  if (/^(run\s+)?smoke$/.test(lower)) return { tool: "smoke", params: {} };
...
export function toolsHelpText(): string {
  const lines = [
