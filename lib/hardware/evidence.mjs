...
async function readContainedJson(rootReal, file, label) {
...
async function verifyPersistedBundle(directory) {
  ...
  if (entries.length !== manifest.size) throw new Error('observation directories do not match the owner manifest');
  for (const entry of entries) {
    if (!entry.isDirectory() || entry.isSymbolicLink() || !manifest.has(entry.name)) {
  ...
  return { valid: true, result: recomputed.result, reasons: recomputed.reasons };
}

/**
 * Reopen and independently verify a persisted evidence bundle.
 *
 * Consumers must call this API before trusting a persisted PASS; `result.json`
 * alone is only a cached summary and is never authoritative.
 */
export async function verifyEvidenceBundle(directory) {
  try {
    return await verifyPersistedBundle(directory);
  } catch (error) {
    return {
      valid: false,
      result: 'FAIL',
