...
function manifestSha256(owner, records, summary, stages = new Map()) {
  ...
  const stageRecords = {};
  for (const [stageId, record] of [...stages].sort(([left], [right]) => left.localeCompare(right))) stageRecords[stageId] = record;
  const canonical = JSON.stringify(canonicalize({ schema: 1, owner, behaviorRecords, stageRecords, summary }));
...
async function verifyOwnedDirectory(rootReal, target, label, expected) {
  const actual = await snapshotOwnedPath(target, 'directory');
  if (expected) assertSameIdentity(expected, actual, target);
...
function normalizeStageDefinitions(value) {
  if (!Array.isArray(value)) throw new TypeError('evidence stages must be an array');
  const stages = new Map();
  for (const entry of value) {
    if (!entry || typeof entry !== 'object' || Array.isArray(entry)) throw new TypeError('evidence stage must be an object');
    assertExactKeys(entry, ['id', 'provider'], 'evidence stage');
    validatePortableComponent(entry.id, 'stage ID', { allowSpaces: false });
    if (stages.has(entry.id)) throw new TypeError(`duplicate evidence stage ${entry.id}`);
    if (typeof entry.provider !== 'string' || entry.provider.length === 0) throw new TypeError('stage provider must be nonempty');
    stages.set(entry.id, { id: entry.id, provider: entry.provider });
  }
  return stages;
}

async function normalizeStageRecord(stage, result, redactValues, root) {
  if (!result || typeof result !== 'object' || Array.isArray(result)) throw new TypeError('stage result must be an object');
  const allowed = ['stageId', 'provider', 'level', 'rawEvidenceRefs', 'rawEvidence', 'diagnostics', 'artifactSha256', 'flashedArtifactSha256', 'nativeArtifactSha256', 'surrogateArtifactSha256', 'sharedSourcePaths', 'startedAt', 'endedAt', 'toolVersion'];
  assertExactKeys(result, allowed, 'stage result');
  if (result.stageId !== stage.id || result.provider !== stage.provider) throw new TypeError('stage result binding is invalid');
  if (!RESULT_LEVELS.has(result.level) || result.level === 'not-run') throw new TypeError('stage result level is invalid');
  const rawEvidenceRefs = normalizeReferences(result.rawEvidenceRefs ?? [], 'rawEvidenceRefs');
  const rawEvidence = [];
  for (const reference of rawEvidenceRefs) rawEvidence.push(await inspectEvidenceReference(root, reference));
  const normalized = redactDeep({ ...result, rawEvidenceRefs, rawEvidence, diagnostics: result.diagnostics ?? {} }, redactValues);
  if (result.artifactSha256 !== undefined) normalized.artifactSha256 = normalizeSha256(result.artifactSha256, 'artifactSha256');
  if (result.flashedArtifactSha256 !== undefined) normalized.flashedArtifactSha256 = normalizeSha256(result.flashedArtifactSha256, 'flashedArtifactSha256');
  if (result.nativeArtifactSha256 !== undefined) normalized.nativeArtifactSha256 = normalizeSha256(result.nativeArtifactSha256, 'nativeArtifactSha256');
  if (result.surrogateArtifactSha256 !== undefined) normalized.surrogateArtifactSha256 = normalizeSha256(result.surrogateArtifactSha256, 'surrogateArtifactSha256');
  if (result.sharedSourcePaths !== undefined) normalized.sharedSourcePaths = normalizeReferences(result.sharedSourcePaths, 'sharedSourcePaths', { nonempty: true, maximum: 128 });
  if (result.startedAt !== undefined) normalized.startedAt = normalizeTimestamp(result.startedAt, 'startedAt');
  if (result.endedAt !== undefined) normalized.endedAt = normalizeTimestamp(result.endedAt, 'endedAt');
  if (Buffer.byteLength(JSON.stringify(normalized.diagnostics), 'utf8') > MAX_DIAGNOSTICS_BYTES) throw new TypeError('stage diagnostics exceed size limit');
  return normalized;
}

/** Create a fail-first, behavior-bound evidence bundle. */
export async function createEvidenceBundle(directory, profile, { redactValues = [], stages = [] } = {}) {
  const runStartedMs = Date.now();
  const root = path.resolve(directory);
  const observations = assertProfile(profile);
  const stageDefinitions = normalizeStageDefinitions(stages);
  ...
  for (const [stageId, stage] of stageDefinitions) stageRecords.set(stageId, { stageId, provider: stage.provider, level: 'not-run' });
  const ownerManifest = {
    schema: 1,
    owner: ownerToken,
    ...
    stages: [...stageDefinitions.values()],
  };
  try {
    await fs.mkdir(path.join(staging, 'observations'));
    await fs.mkdir(path.join(staging, 'stages'));
    ...
    for (const [stageId] of stageDefinitions) {
      const stageRoot = containedPath(staging, 'stages', stageId);
      await atomicWriteJson(containedPath(stageRoot, 'result.json'), stageRecords.get(stageId), undefined, MAX_RECORD_BYTES);
    }
  ...
  const stagesRoot = containedPath(root, 'stages');
  ...
  const stageResultPaths = new Map();
  for (const [behaviorId] of observations) {
    const behaviorRoot = containedPath(observationsRoot, behaviorId);
  ...
  for (const [stageId] of stageDefinitions) {
    const stageRoot = containedPath(stagesRoot, stageId);
    stagePaths.set(stageId, stageRoot);
    stageResultPaths.set(stageId, containedPath(stageRoot, 'result.json'));
  }
  const rootReal = await fs.realpath(root);
  const expectedIdentities = {
    root: await snapshotOwnedPath(root, 'directory'),
    observations: await snapshotOwnedPath(observationsRoot, 'directory'),
    stages: await snapshotOwnedPath(stagesRoot, 'directory'),
  ...
  for (const [stageId] of stageDefinitions) {
    expectedIdentities.stageDirectories.set(stageId, await snapshotOwnedPath(stagePaths.get(stageId), 'directory'));
    expectedIdentities.stageResults.set(stageId, await snapshotOwnedPath(stageResultPaths.get(stageId), 'file'));
  }

  async function verifyPath(target, kind, expected) {
    if (kind === 'directory') {
      await verifyOwnedDirectory(rootReal, target, 'evidence ownership directory', expected);
      return;
    }
  ...
  async function verifyOwnership(behaviorId, stageId) {
    ...
    await verifyPath(stagesRoot, 'directory', expectedIdentities.stages);
    ...
    const selectedStages = stageId === undefined ? [...stageDefinitions.keys()] : [stageId];
    for (const id of selectedStages) {
      if (!stagePaths.has(id)) continue;
      await verifyPath(stagePaths.get(id), 'directory', expectedIdentities.stageDirectories.get(id));
      await verifyPath(stageResultPaths.get(id), 'file', expectedIdentities.stageResults.get(id));
    }
  }
  ...
  async function validatePersistedClaims() {
    ...
    for (const [stageId, record] of stageRecords) {
      const persisted = await readContainedJson(rootReal, stageResultPaths.get(stageId), `stage result ${stageId}`, MAX_RECORD_BYTES);
      if (JSON.stringify(persisted) !== JSON.stringify(record)) throw new Error(`on-disk stage claim changed for ${stageId}`);
      for (let index = 0; index < (record.rawEvidenceRefs ?? []).length; index += 1) {
        const current = await inspectEvidenceReference(root, record.rawEvidenceRefs[index]);
        const expected = record.rawEvidence[index];
        if (current.sha256 !== expected.sha256 || current.size !== expected.size) throw new Error(`raw stage evidence changed for ${stageId}: ${current.path}`);
      }
    }
  }
  ...
  return Object.freeze({
    root,
    async recordStage(stageId, result) {
      return serializeWrite(async () => {
        if (finalized) throw new Error('evidence bundle is finalized');
        const stage = stageDefinitions.get(stageId);
        if (!stage) throw new TypeError(`unknown stage ${stageId}`);
        await verifyOwnership(undefined, stageId);
        const record = await normalizeStageRecord(stage, result, redactValues, root);
        await atomicWriteJson(stageResultPaths.get(stageId), record, () => verifyOwnership(undefined, stageId), MAX_RECORD_BYTES);
        expectedIdentities.stageResults.set(stageId, await snapshotOwnedPath(stageResultPaths.get(stageId), 'file'));
        stageRecords.set(stageId, record);
        return record;
      });
    },
    ...
    async finalize() {
      return serializeWrite(async () => {
        ...
        finalized = true;
        return { ...summary, manifestSha256: manifestSha256(ownerManifest, records, summary, stageRecords) };
      });
    },
  });
}
...
async function verifyPersistedBundle(directory) {
  ...
  assertExactKeys(owner, ['schema', 'owner', 'createdAt', 'targetIdentity', 'behaviors', 'stages'], 'owner marker');
  ...
  if (!Array.isArray(owner.stages)) throw new Error('owner stage manifest is invalid');
  ...
  const stageManifest = normalizeStageDefinitions(owner.stages);

  const observationsRoot = containedPath(root, 'observations');
  const observationsIdentity = await verifyOwnedDirectory(rootReal, observationsRoot, 'observations root');
  const entries = await fs.readdir(observationsRoot, { withFileTypes: true });
  await verifyOwnedDirectory(rootReal, observationsRoot, 'observations root', observationsIdentity);
  ...
  const stagesRoot = containedPath(root, 'stages');
  const stagesIdentity = await verifyOwnedDirectory(rootReal, stagesRoot, 'stages root');
  const stageEntries = await fs.readdir(stagesRoot, { withFileTypes: true });
  await verifyOwnedDirectory(rootReal, stagesRoot, 'stages root', stagesIdentity);
  if (stageEntries.length !== stageManifest.size) throw new Error('stage directories do not match the owner manifest');
  const stageRecords = new Map();
  for (const entry of stageEntries) {
    if (!entry.isDirectory() || entry.isSymbolicLink() || !stageManifest.has(entry.name)) throw new Error(`unexpected stage directory: ${entry.name}`);
  }
  for (const [stageId, stage] of stageManifest) {
    await verifyOwnedDirectory(rootReal, stagesRoot, 'stages root', stagesIdentity);
    const record = await readContainedJson(rootReal, containedPath(stagesRoot, stageId, 'result.json'), `stage result ${stageId}`, MAX_RECORD_BYTES);
    if (record.level === 'not-run') {
      if (JSON.stringify(record) !== JSON.stringify({ stageId, provider: stage.provider, level: 'not-run' })) throw new Error(`not-run stage record was mutated: ${stageId}`);
    } else {
      const normalized = await normalizeStageRecord(stage, record, [], root);
      if (JSON.stringify(normalized) !== JSON.stringify(record)) throw new Error(`stage result provenance changed: ${stageId}`);
    }
    stageRecords.set(stageId, record);
  }
  const authenticatedReferences = new Set();
  for (const record of [...records.values(), ...stageRecords.values()]) {
    for (const reference of record.rawEvidenceRefs ?? []) authenticatedReferences.add(reference);
  }
  for (const entry of entries.filter((candidate) => candidate.isFile())) {
    const reference = `observations/${entry.name}`;
    if (!authenticatedReferences.has(reference)) throw new Error(`unreferenced raw evidence file: ${reference}`);
  }
  ...
  return {
    ...
    manifestSha256: manifestSha256(owner, records, recomputed, stageRecords),
