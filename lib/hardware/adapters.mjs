...
function simulatorScript(profile, artifact) {
  const assertions = profile.observations
    .filter((observation) => observation.provider === 'serial' && typeof observation.contains === 'string')
    .map((observation) => `  - uart_contains: ${yamlString(observation.contains)}`);
...
export function createTrustedAdapters(dependencies = {}) {
  ...
  function buildAdapter(provider, toolName, makeArgs, validate = () => {}) {
    return Object.freeze({
      async preflight(profile) {
        ...
        return Object.freeze({ executable, toolVersion });
      },
      plan(profile, prepared) {
        assertProfile(profile);
        if (!prepared?.executable) throw new TypeError('adapter preflight result is required');
        return launch(prepared.executable, makeArgs(profile), profile.build.workspace, environment);
      },
  ...
  const twin = Object.freeze({
    async preflight(profile) {
      ...
      return Object.freeze({ executable, toolVersion: await getVersion('labwired-sim', executable, profile.build.workspace) });
    },
    plan(profile, prepared, runtime) {
      return launch(prepared.executable, ['test', '--script', runtime.script, '--output-dir', runtime.output, '--no-uart-stdout'], profile.build.workspace, environment);
    },
    async execute(profile, options = {}) {
      let temporary;
      try {
        ...
        const scriptBody = simulatorScript(profile, selected.file);
        ...
        await fs.writeFile(script, scriptBody, { encoding: 'utf8', flag: 'wx', mode: 0o600 });
        await fs.mkdir(output, { mode: 0o700 });
        const descriptor = this.plan(profile, prepared, { script, output });
        ...
        const genuinelyObserved = observed.status === 'pass'
          && Array.isArray(observed.assertions) && observed.assertions.length > 0
          && observed.assertions.every((assertion) => assertion?.passed === true)
