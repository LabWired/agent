...
function safePhysicalEnvironment(environment, additions = {}) {
  const selected = {};
  for (const name of ['PATH', 'PATHEXT', 'SystemRoot', 'WINDIR', 'ComSpec', 'HOME', 'USERPROFILE', 'LANG', 'LC_ALL', 'LC_CTYPE', 'TMP', 'TEMP', 'TMPDIR']) {
    if (typeof environment[name] === 'string') selected[name] = environment[name];
  }
  return Object.freeze({ ...selected, ...additions });
}

function portableToolEnvironment(environment) {
  const canonicalRuntimeNames = new Map([
  ...
  for (const [name, value] of Object.entries(environment).sort(([left], [right]) => left.localeCompare(right))) {
    // npm can export package metadata whose keys contain hyphens. Such names
    // are not portable process-environment identifiers and are never required
    // by vendor toolchains, so discard them before constructing descriptors.
    if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(name) || typeof value !== 'string') continue;
    const folded = name.toLowerCase();
    const existing = aliases.get(folded);
    if (existing && existing.value !== value) throw new TypeError(`conflicting environment aliases ${existing.name} and ${name}`);
    if (!existing) aliases.set(folded, { name, value });
  }
  const selected = {};
  for (const [folded, entry] of aliases) selected[canonicalRuntimeNames.get(folded) ?? entry.name] = entry.value;
...
export function createTrustedAdapters(dependencies = {}) {
  const environment = portableToolEnvironment(dependencies.env ?? process.env);
  ...
  const snapshotToolIdentity = async (executable) => {
    ...
    try {
      ...
      try {
        ...
        if (after.dev !== details.dev || after.ino !== details.ino || after.size !== details.size || after.mtimeMs !== details.mtimeMs) {
          throw new Error(`trusted executable changed while hashing: ${executable}`);
        }
        return { real, dev: details.dev, ino: details.ino, size: details.size, mtimeMs: details.mtimeMs, ctimeMs: details.ctimeMs, sha256: hash.digest('hex') };
  ...
  const toolIdentityFingerprint = (identity) => createHash('sha256').update(JSON.stringify(identity), 'utf8').digest('hex');
  ...
  function buildAdapter(provider, toolName, makeArgs, validate = () => {}) {
    ...
    const adapter = {
      async preflight(profile) {
        ...
        const toolIdentity = await snapshotToolIdentity(executable);
        const prepared = Object.freeze({ executable, toolVersion, toolIdentityFingerprint: toolIdentityFingerprint(toolIdentity) });
        capabilities.set(prepared, Object.freeze({ executable, toolVersion, toolIdentity, fingerprint: buildFingerprint(profile) }));
        return prepared;
      },
  ...
  const prebuiltCapabilities = new WeakMap();
  const prebuilt = Object.freeze({
    async preflight(profile) {
      assertProfile(profile);
      if (profile.build.provider !== 'prebuilt') throw new TypeError('build provider must be prebuilt');
      const artifact = await snapshotFile(profile.build.artifact, profile.build.workspace);
      if (!artifact) throw new Error('prebuilt artifact is absent');
      const prepared = Object.freeze({ provider: 'prebuilt', artifactSha256: artifact.sha256 });
      prebuiltCapabilities.set(prepared, Object.freeze({ fingerprint: buildFingerprint(profile), artifact }));
      return prepared;
    },
    plan(profile, prepared) {
      const capability = prebuiltCapabilities.get(prepared);
      if (!capability || capability.fingerprint !== buildFingerprint(profile)) throw new TypeError('adapter-owned prebuilt capability is required');
      return null;
    },
    async execute(profile, options = {}) {
      const startedAt = new Date().toISOString();
      try {
        const prepared = options.prepared ?? await this.preflight(profile);
        this.plan(profile, prepared);
        const capability = prebuiltCapabilities.get(prepared);
        const current = await snapshotFile(profile.build.artifact, profile.build.workspace);
        if (!sameSnapshot(capability.artifact, current)) throw new Error('prebuilt artifact changed after import planning');
        const endedAt = new Date().toISOString();
        const rawEvidenceRefs = options.evidenceDir ? await persistPhysicalEvidence(
          options.evidenceDir, `import-${profile.target.id}`,
          { provider: 'prebuilt', artifact: current.file, artifactSha256: current.sha256, target: profile.target.id, startedAt, endedAt, classification: 'imported-not-compiled' },
          options.redact, dependencies.physicalEvidenceHooks,
        ) : [];
        return { level: 'imported', provider: 'prebuilt', artifact: current.file, artifactSha256: current.sha256,
          targetIdentity: profile.target, startedAt, endedAt, toolVersion: 'artifact-import-v1', rawEvidenceRefs };
      } catch (error) { return { level: 'failed', provider: 'prebuilt', diagnostics: error.message }; }
    },
  });
  ...
  const twin = Object.freeze({
    async preflight(profile) {
      ...
      const toolIdentity = await snapshotToolIdentity(executable);
      const prepared = Object.freeze({ executable, toolVersion: await getVersion('labwired-sim', executable, profile.build.workspace), toolIdentityFingerprint: toolIdentityFingerprint(toolIdentity) });
    ...
    async execute(profile, options = {}) {
      ...
      try {
        ...
        const selectedArtifact = relation === 'surrogate'
          ? contained(profile.twin.artifact ?? options.twinArtifact, profile.build.workspace, 'surrogate artifact')
          : nativeArtifact;
        ...
        if (relation === 'surrogate') {
          ...
          sharedSources = await snapshotSharedSources(profile.twin.sharedSources ?? options.sharedSourcePaths, profile.build.workspace, snapshotFile);
        }
  ...
  function flashAdapter(provider) {
    ...
    return Object.freeze({
      async preflightPlan(profile) {
        ...
        const toolIdentity = await snapshotToolIdentity(agentPath);
        const prepared = Object.freeze({ provider, executable: agentPath, toolIdentityFingerprint: toolIdentityFingerprint(toolIdentity) });
        planCapabilities.set(prepared, Object.freeze({ fingerprint: physicalFingerprint(profile), toolIdentity }));
        return prepared;
      },
  ...
  function commandObservationAdapter(provider) {
    ...
    return Object.freeze({
      async preflight(profile, observation) {
        ...
        const toolIdentity = await snapshotToolIdentity(agentPath);
        const prepared = Object.freeze({ provider, executable: agentPath, toolIdentityFingerprint: toolIdentityFingerprint(toolIdentity) });
        capabilities.set(prepared, Object.freeze({ fingerprint: physicalFingerprint(profile, { observation }), toolIdentity }));
        return prepared;
      },
  ...
  const logicCapabilities = new WeakMap();
  const logicRuntimeCapabilities = new WeakMap();
  const logicCsv = Object.freeze({
    async preflight(profile, observation) {
      assertProfile(profile);
      if (observation?.provider !== 'logic-csv') throw new TypeError('observation provider must be logic-csv');
      if (observation.requiredLevel !== 'hardware_observed') return Object.freeze({ provider: 'logic-csv', trust: 'imported' });
      if (observation.captureProvider !== 'sigrok-cli') throw new Error('BLOCKED: hardware logic observation requires trusted sigrok-cli capture');
      if (!PHYSICAL_LOGIC_DRIVERS.has(String(observation.driver).toLowerCase())) throw new Error('BLOCKED: synthetic or unregistered logic driver cannot prove physical behavior');
      if (!/^[A-Za-z0-9][A-Za-z0-9._\/-]*$/.test(String(observation.instrumentId))) throw new Error('BLOCKED: unsafe sigrok connection identity');
      const executable = await resolveTool('sigrok-cli');
      if (typeof executable !== 'string' || !path.isAbsolute(executable)) throw new TypeError('sigrok-cli must resolve to an explicit absolute executable');
      const toolIdentity = await snapshotToolIdentity(executable);
      const toolVersion = await getVersion('sigrok-cli', executable, profile.build.workspace);
      const prepared = Object.freeze({ provider: 'logic-csv', captureProvider: 'sigrok-cli', executable, toolVersion, instrumentId: observation.instrumentId, toolIdentityFingerprint: toolIdentityFingerprint(toolIdentity) });
      logicCapabilities.set(prepared, Object.freeze({ fingerprint: physicalFingerprint(profile, { observation }), toolIdentity }));
      return prepared;
    },
    plan(profile, observation, prepared, runtime) {
      const capability = logicCapabilities.get(prepared);
      if (!capability || capability.fingerprint !== physicalFingerprint(profile, { observation })) throw new TypeError('adapter-owned logic capture capability is required');
      const capture = logicRuntimeCapabilities.get(runtime);
      if (!capture) throw new TypeError('adapter-owned private capture destination is required');
      const samples = Math.ceil(observation.sampleRateHz * observation.durationSeconds);
      return launch(prepared.executable, [
        '--driver', `${observation.driver}:conn=${observation.instrumentId}`,
        '--channels', observation.sourceChannel,
        '--config', `samplerate=${observation.sampleRateHz}`,
        '--samples', String(samples),
        '--output-format', 'csv', '--output-file', capture.file,
      ], profile.build.workspace, safePhysicalEnvironment(environment));
    },
    async execute(profile, observation, options = {}) {
      let temporary;
      try {
        const prepared = options.prepared ?? await this.preflight(profile, observation);
        if (observation.requiredLevel !== 'hardware_observed') {
          if (prepared?.trust !== 'imported' || !observation.file) throw new TypeError('adapter-owned imported logic capability is required');
          const imported = await snapshotFile(observation.file, profile.build.workspace, { captureBytes: true, maximumBytes: MAX_PHYSICAL_EVIDENCE_BYTES, label: 'imported logic CSV' });
          if (!imported) throw new Error('imported logic CSV is absent');
          const analysis = analyzeLogicCsv(imported.bytes.toString('utf8'), observation);
          return { level: 'untrusted_observation', provider: 'logic-csv', captureSha256: imported.sha256, ...analysis, diagnostics: 'pre-existing CSV was parsed as untrusted and cannot prove hardware behavior' };
        }
        const capability = logicCapabilities.get(prepared);
        if (!capability) throw new TypeError('adapter-owned trusted logic capture capability is required');
        await verifyToolIdentity(prepared.executable, capability.toolIdentity);
        temporary = await fs.mkdtemp(path.join(os.tmpdir(), 'labwired-logic-capture-'));
        const captureFile = path.join(temporary, 'capture.csv');
        try { await fs.lstat(captureFile); throw new Error('private logic capture output unexpectedly pre-exists'); } catch (error) { if (error?.code !== 'ENOENT') throw error; }
        const runtime = Object.freeze({ file: captureFile });
        logicRuntimeCapabilities.set(runtime, runtime);
        const descriptor = this.plan(profile, observation, prepared, runtime);
        const startedAt = new Date().toISOString();
        const processResult = await executeLaunch(descriptor, { timeoutMs: (observation.timeoutSeconds ?? Math.ceil(observation.durationSeconds + 5)) * 1000, signal: options.signal, redact: options.redact, onDelta: options.onDelta });
        const endedAt = new Date().toISOString();
        if (!processSucceeded(processResult)) throw new Error(diagnostics(processResult) || `trusted logic capture ${processResult?.classification ?? 'failed'}`);
        const snapshot = await snapshotFile(captureFile, temporary, {
          captureBytes: true, maximumBytes: MAX_PHYSICAL_EVIDENCE_BYTES, label: 'logic CSV',
        });
        if (!snapshot) throw new Error('trusted provider did not create a logic CSV capture');
        ...
        const after = await snapshotFile(captureFile, temporary);
        if (!sameSnapshot(snapshot, after)) throw new Error('logic CSV capture changed during observation');
        const artifactSha256 = await bindFlashedArtifact(profile, options);
        const capture = { provider: 'logic-csv', captureProvider: 'sigrok-cli', instrumentId: observation.instrumentId, driver: observation.driver, sourceChannel: observation.sourceChannel, requestedSampleRateHz: observation.sampleRateHz, requestedDurationSeconds: observation.durationSeconds, requestedSamples: Math.ceil(observation.sampleRateHz * observation.durationSeconds), artifactSha256, captureSha256: createHash('sha256').update(rawText).digest('hex'), toolVersion: prepared.toolVersion, toolIdentityFingerprint: prepared.toolIdentityFingerprint, command: { executable: descriptor.executable, args: descriptor.args, shell: false }, process: processResult, startedAt, endedAt, ...analysis };
      ...
      } catch (error) { return redactDeep({ level: /^BLOCKED:/.test(error.message) ? 'blocked' : 'failed', provider: 'logic-csv', diagnostics: error.message }, options.redact); }
  ...
  const network = Object.freeze({
    async preflight(profile, observation) {
      ...
      const toolIdentity = await snapshotToolIdentity(agentPath);
      const prepared = Object.freeze({ provider: 'network', executable: agentPath, toolIdentityFingerprint: toolIdentityFingerprint(toolIdentity) });
      networkCapabilities.set(prepared, toolIdentity);
      return prepared;
    },
    async execute(profile, observation, options = {}) {
  ...
  return Object.freeze({
    build: Object.freeze({ platformio, make, cmake, prebuilt }),
