...
export function createTrustedAdapters(dependencies = {}) {
    ...
    return Object.freeze({
      ...
      plan(profile, prepared) {
        ...
        return launch(agentPath, [
          'probe', 'flash', profile.build.artifact, '--chip', profile.target.chip,
          '--target', 'probe', '--probe', profile.target.probeSerial,
        ], profile.build.workspace, safePhysicalEnvironment(environment, {
          LABWIRED_HW_PORT: profile.target.serialPort,
          LABWIRED_FLASH_PROVIDER: provider,
        }));
      },
      async execute(profile, options = {}) {
        try {
  ...
  const logicCsv = Object.freeze({
    ...
    async execute(profile, observation, options = {}) {
      try {
        ...
        const analysis = analyzeLogicCsv(snapshot.bytes.toString('utf8'), observation);
        ...
        const artifactSha256 = await bindFlashedArtifact(profile, options);
        const capture = { provider: 'logic-csv', artifactSha256, captureSha256: snapshot.sha256, ...analysis };
        const rawEvidenceRefs = await persistPhysicalEvidence(options.evidenceDir, observation.id, capture, options.redact);
  ...
  const network = Object.freeze({
    ...
    async execute(profile, observation, options = {}) {
      ...
      try {
        await this.preflight(profile, observation);
        if (typeof options.deviceCapture !== 'string') throw new Error('network observation requires a device serial capture');
        const escapedMarker = observation.deviceMarker.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const escapedKey = observation.hostProbeUrlFromMarker.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const match = options.deviceCapture.match(new RegExp(`${escapedMarker}[^\\r\\n]*\\bnonce=([^\\s]+)[^\\r\\n]*\\b${escapedKey}=([^\\s]+)`));
        ...
        const artifactSha256 = await bindFlashedArtifact(profile, options);
        const capture = redactDeep({ provider: 'network', nonce, address: host.address, port: host.port, artifactSha256, statusCode: response.statusCode, response: response.body }, options.redact);
        const rawEvidenceRefs = await persistPhysicalEvidence(options.evidenceDir, observation.id, capture, options.redact);
