...
function safePhysicalEnvironment(environment, additions = {}) {
  const selected = {};
  for (const name of ['PATH', 'PATHEXT', 'SystemRoot', 'WINDIR', 'HOME', 'USERPROFILE', 'LANG', 'LC_ALL', 'TMP', 'TEMP']) {
  ...
  const logicCsv = Object.freeze({
    ...
    async execute(profile, observation, options = {}) {
      ...
      try {
        ...
        const artifactSha256 = await bindFlashedArtifact(profile, options);
        const capture = { provider: 'logic-csv', captureProvider: 'sigrok-cli', instrumentId: observation.instrumentId, driver: observation.driver, sourceChannel: observation.sourceChannel, sampleRateHz: observation.sampleRateHz, durationSeconds: observation.durationSeconds, artifactSha256, captureSha256: createHash('sha256').update(rawText).digest('hex'), toolVersion: prepared.toolVersion, toolIdentityFingerprint: prepared.toolIdentityFingerprint, command: { executable: descriptor.executable, args: descriptor.args, shell: false }, process: processResult, startedAt, endedAt, ...analysis };
