...
async function persistPhysicalEvidence(evidenceDir, observationId, value, redactValues = []) {
  if (!evidenceDir) return [];
  const root = path.resolve(evidenceDir);
  const rootDetails = await fs.lstat(root);
  if (rootDetails.isSymbolicLink() || !rootDetails.isDirectory()) throw new Error('evidenceDir must be a real directory');
  ...
  const directoryDetails = await fs.lstat(directory);
  if (directoryDetails.isSymbolicLink() || !directoryDetails.isDirectory()) throw new Error('observation evidence path must be a real directory');
  contained(await fs.realpath(directory), await fs.realpath(root), 'observation evidence path');
  const reference = `observations/${observationId}.json`;
  const output = path.join(root, reference);
  const body = `${JSON.stringify(redactDeep(value, redactValues), null, 2)}\n`;
  if (Buffer.byteLength(body) > MAX_PHYSICAL_EVIDENCE_BYTES) throw new Error('physical evidence exceeds the capture size limit');
  const handle = await fs.open(output, fsConstants.O_WRONLY | fsConstants.O_CREAT | fsConstants.O_EXCL | (fsConstants.O_NOFOLLOW ?? 0), 0o400);
  try { await handle.writeFile(body, 'utf8'); await handle.sync(); } finally { await handle.close(); }
  return [reference];
}

async function persistPhysicalBytes(evidenceDir, observationId, extension, value, redactValues = []) {
  if (!evidenceDir) return [];
  const root = path.resolve(evidenceDir);
  const directory = path.join(root, 'observations');
  try { await fs.mkdir(directory, { mode: 0o700 }); } catch (error) { if (error?.code !== 'EEXIST') throw error; }
  const details = await fs.lstat(directory);
  if (details.isSymbolicLink() || !details.isDirectory()) throw new Error('observation evidence path must be a real directory');
  contained(await fs.realpath(directory), await fs.realpath(root), 'observation evidence path');
  ...
  const handle = await fs.open(path.join(root, reference), fsConstants.O_WRONLY | fsConstants.O_CREAT | fsConstants.O_EXCL | (fsConstants.O_NOFOLLOW ?? 0), 0o400);
  try { await handle.writeFile(redacted, 'utf8'); await handle.sync(); } finally { await handle.close(); }
  return [reference];
}
...
export function createTrustedAdapters(dependencies = {}) {
  ...
  const agentPath = dependencies.agentPath ?? path.resolve(path.dirname(new URL(import.meta.url).pathname), '../../bin/labwired-agent');
  ...
  function flashAdapter(provider) {
    ...
    return Object.freeze({
      async preflight(profile, options = {}) {
        ...
        capabilities.set(prepared, Object.freeze({ fingerprint: physicalFingerprint(profile), artifact }));
        return prepared;
      },
      plan(profile, prepared) {
        const capability = capabilities.get(prepared);
      ...
      async execute(profile, options = {}) {
        try {
          const prepared = await this.preflight(profile, options);
          const capability = capabilities.get(prepared);
          const descriptor = this.plan(profile, prepared);
          const processResult = await executeLaunch(descriptor, {
          ...
          if (receipt.provider !== provider || receipt.artifactSha256 !== after.sha256
            ...
            || receipt.serialPort !== profile.target.serialPort) throw new Error('flash receipt does not match the exact artifact and identities');
          ...
          const rawEvidenceRefs = await persistPhysicalEvidence(options.evidenceDir, `flash-${profile.target.id}`, capture, options.redact);
  ...
  function commandObservationAdapter(provider) {
    ...
    return Object.freeze({
      async preflight(profile, observation) {
        ...
        const prepared = Object.freeze({ provider });
        capabilities.set(prepared, physicalFingerprint(profile, { observation }));
        return prepared;
      },
      plan(profile, observation, prepared) {
        const fingerprint = capabilities.get(prepared);
        if (!fingerprint) throw new TypeError('adapter-owned preflight capability is required');
        if (fingerprint !== physicalFingerprint(profile, { observation })) throw new TypeError('observation inputs changed after preflight');
        const args = provider === 'serial'
          ...
          : ['probe', 'rtt-capture', '--chip', profile.target.chip, '--elf', profile.build.artifact, '--marker', observation.contains, '--timeout', String(observation.timeoutSeconds ?? 8)];
      ...
      async execute(profile, observation, options = {}) {
        try {
          const prepared = await this.preflight(profile, observation);
          ...
          const rawEvidenceRefs = await persistPhysicalEvidence(options.evidenceDir, observation.id, capture, options.redact);
  ...
  const logicCsv = Object.freeze({
    ...
    async execute(profile, observation, options = {}) {
      try {
        await this.preflight(profile, observation);
        ...
        const sourceRefs = await persistPhysicalBytes(options.evidenceDir, observation.id, 'csv', rawText, []);
        const rawEvidenceRefs = [...sourceRefs, ...await persistPhysicalEvidence(options.evidenceDir, observation.id, capture, options.redact)];
  ...
  const network = Object.freeze({
    async preflight(profile, observation) {
      ...
      return Object.freeze({ provider: 'network' });
    },
    async execute(profile, observation, options = {}) {
      ...
      try {
        ...
        const deviceRefs = await persistPhysicalBytes(options.evidenceDir, `${observation.id}-device`, 'txt', challengeCapture, options.redact);
        const hostRefs = await persistPhysicalBytes(options.evidenceDir, `${observation.id}-host`, 'txt', response.body, options.redact);
        const rawEvidenceRefs = [...deviceRefs, ...hostRefs, ...await persistPhysicalEvidence(options.evidenceDir, observation.id, capture, options.redact)];
