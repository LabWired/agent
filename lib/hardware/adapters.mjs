...
async function readSimulatorResult(file, temporaryRoot) {
  const snapshot = await artifactSnapshot(file, temporaryRoot);
  if (!snapshot) throw new Error('simulator did not publish result.json');
  if (Number(snapshot.size) > MAX_SIMULATOR_RESULT_BYTES) throw new Error('simulator result.json exceeds the capture size limit');
  const handle = await fs.open(file, fsConstants.O_RDONLY | (fsConstants.O_NOFOLLOW ?? 0));
  try {
    const details = await handle.stat();
    if (!details.isFile() || details.size > MAX_SIMULATOR_RESULT_BYTES) throw new Error('simulator result.json exceeds the capture size limit');
    const text = await handle.readFile('utf8');
    if (Buffer.byteLength(text) > MAX_SIMULATOR_RESULT_BYTES) throw new Error('simulator result.json exceeds the capture size limit');
    let parsed;
    try { parsed = JSON.parse(text); } catch { throw new Error('simulator did not publish a valid result.json'); }
...
async function persistSimulatorEvidence(bundle, observed, redactValues) {
  ...
  try {
    ...
    const persisted = await artifactSnapshot(finalPath, bundle.root);
  ...
  const twin = Object.freeze({
    ...
    async execute(profile, options = {}) {
      ...
      try {
        ...
        const observed = await readSimulatorResult(path.join(output, 'result.json'), temporary);
        ...
        const rawEvidenceRef = await persistSimulatorEvidence(evidenceBundle, observed, options.redact);
        if (relation === 'exact') {
          return {
