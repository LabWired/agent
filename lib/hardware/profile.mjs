...
const SIGROK_CONNECTION_ID = /^[A-Za-z0-9][A-Za-z0-9._\/-]*$/;
...
export function containsInlineCredential(value) {
...
function normalizeTwin(value, build) {
  ...
  exactKeys(twin, ['provider', 'system', 'artifactRelation', 'artifact', 'sharedSources', 'timeoutSeconds'], 'twin');
  ...
  if (artifactRelation === 'surrogate') {
    normalized.artifact = containedPath(twin.artifact, build.workspace, build.workspace, 'twin.artifact');
    if (!Array.isArray(twin.sharedSources) || twin.sharedSources.length === 0) fail('twin.sharedSources', 'must be a non-empty array');
    normalized.sharedSources = twin.sharedSources.map((item, index) => containedPath(item, build.workspace, build.workspace, `twin.sharedSources[${index}]`));
  } else if (twin.artifact !== undefined || twin.sharedSources !== undefined) {
    fail('twin', 'exact relation must not declare a separate artifact or sharedSources');
  }
  return normalized;
}
...
function normalizeObservation(value, index, build) {
  ...
  const keys = {
    ...
    'logic-csv': ['id', 'provider', 'file', 'channel', 'timeColumn', 'valueColumn', 'edgeCountAtLeast', 'frequencyMinHz', 'frequencyMaxHz', 'captureProvider', 'instrumentId', 'driver', 'sourceChannel', 'sampleRateHz', 'durationSeconds', 'timeoutSeconds', 'requiredLevel'],
  ...
  if (selectedProvider === 'serial' || selectedProvider === 'rtt') {
    ...
    if (observation.file !== undefined) normalized.file = containedPath(observation.file, build.workspace, build.workspace, `${location}.file`);
    ...
    const captureKeys = ['captureProvider', 'instrumentId', 'driver', 'sourceChannel', 'sampleRateHz', 'durationSeconds'];
    const hasCapture = captureKeys.some((key) => observation[key] !== undefined);
    if (!hasCapture && observation.file === undefined) fail(`${location}.file`, 'is required for an imported untrusted observation');
    if (hasCapture && observation.file !== undefined) fail(`${location}.file`, 'must not be used by a trusted provider-owned capture');
    if (hasCapture) {
      if (observation.captureProvider !== 'sigrok-cli') fail(`${location}.captureProvider`, 'must equal trusted provider sigrok-cli');
      normalized.captureProvider = 'sigrok-cli';
      normalized.instrumentId = identity(observation.instrumentId, `${location}.instrumentId`);
      if (!SIGROK_CONNECTION_ID.test(normalized.instrumentId)) fail(`${location}.instrumentId`, 'contains characters unsafe for an exact sigrok conn selector');
      normalized.driver = safeId(observation.driver, `${location}.driver`);
      if (!PHYSICAL_LOGIC_DRIVERS.has(normalized.driver.toLowerCase())) fail(`${location}.driver`, 'must name an allowlisted physical logic analyzer driver');
      normalized.sourceChannel = identity(observation.sourceChannel, `${location}.sourceChannel`);
      if (!Number.isInteger(observation.sampleRateHz) || observation.sampleRateHz < 1 || observation.sampleRateHz > 100_000_000) fail(`${location}.sampleRateHz`, 'must be a bounded positive integer');
      if (!Number.isFinite(observation.durationSeconds) || observation.durationSeconds <= 0 || observation.durationSeconds > MAX_TIMEOUT_SECONDS) fail(`${location}.durationSeconds`, 'must be positive and bounded');
      if (observation.sampleRateHz * observation.durationSeconds > 100_000_000) fail(location, 'capture sample count must be bounded');
      normalized.sampleRateHz = observation.sampleRateHz;
      normalized.durationSeconds = observation.durationSeconds;
    }
    for (const key of ['frequencyMinHz', 'frequencyMaxHz']) {
      if (observation[key] !== undefined) {
        if (!Number.isFinite(observation[key]) || observation[key] <= 0) fail(`${location}.${key}`, 'frequency must be positive and finite');
        normalized[key] = observation[key];
      }
    }
    if (normalized.frequencyMinHz !== undefined && normalized.frequencyMaxHz !== undefined
      && normalized.frequencyMinHz > normalized.frequencyMaxHz) fail(location, 'minimum frequency must not exceed maximum frequency');
  } else {
...
export function validateHardwareProfile(value, sourcePath) {
  ...
  for (const observation of observations) {
    ...
    if (observation.provider === 'logic-csv' && observation.captureProvider !== 'sigrok-cli') {
      fail(`observations.${observation.id}`, 'hardware_observed logic requires trusted capture configuration');
    }
  }
  return deepFreeze({
    schema: 1,
