...
function requireString(value, location) {
  ...
  if (SECRET_ASSIGNMENT.test(value) || SECRET_VALUE.test(value)) {
...
function normalizeTarget(value) {
  ...
  return {
    id: safeId(target.id, 'target.id'),
    chip: requireString(target.chip, 'target.chip'),
    ...(target.probeSerial === undefined ? {} : { probeSerial: requireString(target.probeSerial, 'target.probeSerial') }),
    ...(target.serialPort === undefined ? {} : { serialPort: requireString(target.serialPort, 'target.serialPort') }),
  };
}
...
export function validateHardwareProfile(value, sourcePath) {
  ...
  for (const observation of observations) {
    ...
    if (observation.provider === 'serial' && !target.serialPort) {
      fail('target', 'physical serial observation requires explicit serialPort identity');
    }
    if (observation.provider === 'rtt' && !target.probeSerial) {
      fail('target', 'physical RTT observation requires explicit probeSerial identity');
    }
  }
  return deepFreeze({
    schema: 1,
...
export function canonicalProfile(profile) {
  ...
  if (!isRecord(profile)) return typeof profile === 'string' && SECRET_ASSIGNMENT.test(profile) ? '[REDACTED]' : profile;
  const canonical = {};
