...
async function cleanupFailedCreation(created) {
  ...
  const content = await readFile(created.lockPath, 'utf8');
  if (!content.includes(`\"token\":\"${created.token}\"`)) {
    throw new Error('Refusing to clean up an unproven hardware lock after initialization failure');
  }
  const quarantine = `${created.lockPath}.failed-${randomBytes(16).toString('hex')}`;
  await rename(created.lockPath, quarantine);
  const moved = await lstat(quarantine);
  const movedContent = await readFile(quarantine, 'utf8');
  if (moved.dev !== created.dev || moved.ino !== created.ino
    || !movedContent.includes(`\"token\":\"${created.token}\"`)) {
    try { await rename(quarantine, created.lockPath); } catch { /* Preserve evidence. */ }
...
async function acquireOne(rootState, entry, owner, hooks) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      ...
      created = { lockPath, dev: initialInfo.dev, ino: initialInfo.ino, token: record.token };
