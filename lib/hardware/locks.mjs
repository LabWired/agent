...
function validateIdentities(identities) {
  ...
  const canonical = entries.map(([type, identity]) => [type.startsWith('instrument-') ? 'instrument' : type, identity]);
  return [...new Map(canonical.map((entry) => [`${entry[0]}\0${entry[1]}`, entry])).values()]
