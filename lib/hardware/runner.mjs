...
async function prepare({ profilePath, evidenceDir, dependencies }) {
  ...
  const rawPlan = {
    ...
    evidenceDir,
    operationalFingerprint: digest({ profile: canonicalProfile(profile), evidenceDir, capabilities: safeCapabilities }),
  ...
  return { plan, digest: planDigest, profile, deps, selected, identities, capabilities: { build: buildCapability, twin: twinCapability, flashPlan: flashPlanCapability, observations: preparedObservations } };
}
...
export async function executeHardwareRun(input) {
  ...
  const { profile, deps, selected, identities, capabilities } = prepared;
  ...
  const evidence = await deps.createEvidence(input.evidenceDir, profile, { redactValues: deps.redactValues, stages: stageDefinitions });
  let locks;
  let executionError;
  let halted = false;
  ...
  try {
    if (profile.flash || profile.observations.some(isHardwareObservation)) {
      locks = await deps.acquireLocks(identities.stableIds, { root: deps.lockRoot, signal: input.signal });
    }
    let buildResult;
    const buildStartedAt = new Date().toISOString();
    try {
      buildResult = await selected.build.execute(profile, { prepared: capabilities.build, evidenceDir: input.evidenceDir, signal: input.signal, redact: deps.redactValues, onDelta: deps.onDelta });
    } catch (error) {
