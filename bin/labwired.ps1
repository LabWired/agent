...
<#
  ...
  Installed under $LABWIRED_HOME\bin and $LABWIRED_HOME\agent\bin
#>
param(
...
Usage:
  labwired                 Start agent (OpenCode)
  labwired doctor          Check install
  labwired update          Self-update kit + tools (Cursor-style)
  labwired version         Version
  labwired package info    Portable prefix info
  labwired package path    Print LABWIRED_HOME
  labwired install-deps    Refresh tools into prefix
  labwired help
...
function Cmd-Package {
  ...
  switch ($sub) {
    ...
    "uninstall" {
      if ($argsRest -notcontains "--yes") { Fail "re-run: labwired package uninstall --yes" }
      $userBin = Join-Path $env:LOCALAPPDATA "LabWired\bin"
      Remove-Item (Join-Path $userBin "labwired.cmd") -ErrorAction SilentlyContinue
      Remove-Item (Join-Path $HomeDir "agent") -Recurse -Force -ErrorAction SilentlyContinue
      Remove-Item (Join-Path $HomeDir "tools") -Recurse -Force -ErrorAction SilentlyContinue
      Remove-Item (Join-Path $HomeDir "bin") -Recurse -Force -ErrorAction SilentlyContinue
      Remove-Item (Join-Path $HomeDir "cache") -Recurse -Force -ErrorAction SilentlyContinue
      Remove-Item (Join-Path $HomeDir "env.ps1") -Force -ErrorAction SilentlyContinue
      Remove-Item (Join-Path $HomeDir "MANIFEST.json") -Force -ErrorAction SilentlyContinue
      Say "uninstalled portable package from $HomeDir"
    }
    default {
...
function Cmd-Update {
  ...
  & $install -Prefix $HomeDir -Full
  Say "update complete — run: labwired doctor"
}
