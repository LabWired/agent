...
need 'temporal.*GPIO.*labwired_verify.*direct|labwired_verify.*direct.*temporal.*GPIO' 'temporal GPIO arms verification before execution'
need 'output.*summary.*never.*full|never.*full.*output.*summary' 'temporal verification keeps bounded evidence'
